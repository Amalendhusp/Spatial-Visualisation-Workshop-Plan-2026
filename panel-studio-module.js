...
<div id="screen-workshop">
  ...
  <div class="studio-header">
    ...
    <button class="btn ghost" id="btn-fullscreen" onclick="toggleWorkspaceFullscreen()" title="More room to arrange panels"><i data-lucide="maximize-2"></i> Full Screen</button>
  ...
  <div class="ws-body">
    ...
    <div class="right-panel">
        ...
        <div class="workspace-card">
          ...
          <div class="ws-flex">
            ...
            <div class="icon-toolbar left">
              ...
              <button id="btn-point" class="icon-btn" onclick="togglePointTool()" title="Point Edit tool"><i data-lucide="spline"></i><span class="icon-label">Point Edit tool — click a panel (or double-click one) to edit its points</span></button>
              <div id="point-edit-subtools" class="point-edit-subtools" style="display:none">
                <button id="btn-point-move" class="icon-btn sub on" onclick="setPointEditAction('move')" title="Move points"><i data-lucide="move"></i><span class="icon-label">Move — click an anchor to select it, drag to reposition, click without dragging to toggle straight/curved</span></button>
                <button id="btn-point-add" class="icon-btn sub" onclick="setPointEditAction('add')" title="Add Point"><i data-lucide="circle-plus"></i><span class="icon-label">Add Point — click anywhere on the path to insert a new anchor there</span></button>
                <button id="btn-point-delete" class="icon-btn sub" onclick="setPointEditAction('delete')" title="Delete Point"><i data-lucide="circle-minus"></i><span class="icon-label">Delete Point — click an anchor to remove it</span></button>
              </div>
              ...
              <div class="icon-sep"></div>
              <button id="btn-zoom-in" class="icon-btn" onclick="zoomWorkspace(1)"><i data-lucide="zoom-in"></i><span class="icon-label">Zoom in</span></button>
              <button id="btn-zoom-out" class="icon-btn" onclick="zoomWorkspace(-1)"><i data-lucide="zoom-out"></i><span class="icon-label">Zoom out</span></button>
              <button id="btn-zoom-reset" class="icon-btn ps-zoom-pct" onclick="resetWorkspaceZoom()"><span class="icon-label">Reset zoom</span>100%</button>
              <div class="icon-sep"></div>
              <!-- Emergency stabilization: recovers a stuck interaction state
                   (panels visible but unselectable) WITHOUT touching any
                   panel/shape — never confuse with Clear Workspace below. -->
              <button id="btn-reset-interaction" class="icon-btn" onclick="resetInteractionState()" title="Reset Interaction"><i data-lucide="refresh-ccw"></i><span class="icon-label">Reset Interaction — if selection stops working, click this. Does NOT delete your work.</span></button>
            </div>
            ...
            <div class="icon-toolbar right ps-toolbar-labeled">
              ...
              <div class="icon-sep"></div>
              <!-- V2 item 12 (Simplify Export): Print and PDF hidden from the
                   participant toolbar — printWorkspace()/exportWorkspace('pdf')
                   stay fully implemented and reachable (e.g. window.printWorkspace()),
                   just not exposed as buttons here. Only PNG remains. -->
      ...
      <div class="ps-canvas-hint" id="toolbar-hint">drag to move · select a panel, then use the tools · scroll the tray for all panels</div>
    </div><!-- /right -->

  </div><!-- /ws-body -->

  <!-- Hint Viewer: pannable/zoomable/dismissible overlay opened by clicking
       a Hint tab — deliberately its own element, independent of
       #garment-overlay (which Map Mode uses for click-to-place pins), so
       drag-to-pan here can never be confused with a pin placement. -->
  <div id="hint-viewer">
    <div class="hint-viewer-topbar">
      <span id="hint-viewer-title"></span>
      <div class="hint-viewer-zoom-controls">
        <button type="button" onclick="hintViewerZoomBy(1/1.25)" title="Zoom out">−</button>
        <span id="hint-viewer-zoom-pct">100%</span>
        <button type="button" onclick="hintViewerZoomBy(1.25)" title="Zoom in">+</button>
        <button type="button" onclick="hintViewerResetZoom()" title="Reset zoom">Reset</button>
      </div>
      <button type="button" class="hint-viewer-close" onclick="closeHintViewer()">✕ Close</button>
    </div>
    <div class="hint-viewer-canvas" id="hint-viewer-canvas">
      <img id="hint-viewer-img" src="" alt="" draggable="false">
    </div>
  </div>
</div><!-- /screen-workshop -->
...
function _taskStageName(){
  const map={pretest:'Pre-test',s1:'Stage 1',s2:'Stage 2',s3:'Stage 3',posttest:'Post-test',training:'Training Task'};
...
function showCompleteModal(){
  // Stage 2 is constructed physically/in Clo3D, not on this workspace — an
  // empty workspace there is expected, not an error. Training is optional
  // practice — a participant may just be testing tray-insert without
  // tracing anything, which is fine too.
  const isConstructionStage = currentTaskMeta && currentTaskMeta.stage === 's2';
  const isTraining = currentTaskMeta && currentTaskMeta.stage === 'training';
  if (!wsPanels.length && !isConstructionStage && !isTraining) { alert('Your workspace is empty — arrange your panels before completing the task.'); return; }
  // Training deliberately does NOT use the research-stage "Complete {stage}"
  // wording/lock semantics — see PanelStudio.onComplete's isTraining branch
  // in participant-page_3.html (finishTrainingTask(), not stage completion).
  const title = isTraining ? 'Finish Training?' : `Complete ${_taskStageName()}?`;
  const body = isTraining
    ? `This ends the practice activity and returns you to the overview. You can repeat the Training Task anytime.`
    : `Your Studio task will be marked complete — editing tools will be locked, and you'll be able to review your submission afterward.`;
  const confirmLabel = isTraining ? 'Finish Training' : '✓ Yes, mark complete';
  document.getElementById('task-modal-box').innerHTML = `
    ...
    <p style="color:#444;line-height:1.6;margin-top:14px">${body}</p>
    <div style="display:flex;gap:10px;margin-top:20px">
      ...
      <button onclick="_confirmComplete()" style="padding:10px 18px;border-radius:8px;border:1px solid #1a7f37;background:#1a7f37;color:#fff;cursor:pointer;font-weight:600">${confirmLabel}</button>
    </div>
  `;
...
// ── WORKSPACE ZOOM (Workspace Enhancement, item 2) — purely a visual
// transform:scale() on #workspace; p.x/p.y/panel geometry are NEVER
// rescaled, so "panel positions must remain unchanged" holds by
// construction. Every place that turns a mouse/touch client coordinate into
// a workspace-relative one must go through wsPointFromClient()/
// wsDeltaFromClient() below rather than hand-rolling `clientX - wsRect.left`
// — that raw math is only valid at zoom=1. ──
let wsZoom = 1;
const WS_ZOOM_MIN = 0.25, WS_ZOOM_MAX = 5;
// Absolute client point → workspace-logical point (undoes both the
// workspace's on-screen position and its current zoom).
function wsPointFromClient(clientX, clientY){
  const r = document.getElementById('workspace').getBoundingClientRect();
  return { x: (clientX - r.left) / wsZoom, y: (clientY - r.top) / wsZoom };
}
// Raw screen-pixel delta (e.g. e.clientX - dragStartClientX) → workspace-
// logical delta, so a given screen-pixel drag always moves something the
// same logical distance regardless of current zoom.
function wsDeltaFromClient(dxClient, dyClient){
  return { dx: dxClient / wsZoom, dy: dyClient / wsZoom };
}
// Keeps the 4 ruler strips visually aligned with the (possibly scaled,
// possibly scrolled) workspace — combines the existing scroll-sync
// translate with the current zoom scale in one transform. scale() is
// rightmost so it applies first (in the ruler-inner's own local space),
// then translate shifts the already-scaled strip by the raw (already
// zoom-scaled) scroll offset — see panel-studio-module.css for the
// matching transform-origin:0 0 on these elements.
function syncRulerTransforms(){
  const vp = document.getElementById('ws-viewport');
  if (!vp) return;
  const tx = `translateX(${-vp.scrollLeft}px) scale(${wsZoom})`;
  const ty = `translateY(${-vp.scrollTop}px) scale(${wsZoom})`;
  const hEl = document.getElementById('ruler-h-inner'), bEl = document.getElementById('ruler-b-inner');
  const vEl = document.getElementById('ruler-v-inner'), rEl = document.getElementById('ruler-r-inner');
  if (hEl) hEl.style.transform = tx;
  if (bEl) bEl.style.transform = tx;
  if (vEl) vEl.style.transform = ty;
  if (rEl) rEl.style.transform = ty;
}
function applyWsZoomToDOM(){
  const ws = document.getElementById('workspace');
  if (ws) ws.style.transform = `scale(${wsZoom})`;
  syncRulerTransforms();
  const label = document.getElementById('btn-zoom-reset');
  if (label) label.textContent = Math.round(wsZoom*100)+'%';
}
// Sets zoom, keeping the given client point (or the viewport's own center,
// for toolbar-button calls with no cursor position) visually anchored in
// place — the standard "zoom toward a point" technique used by design
// software, so pinch/ctrl+wheel zoom doesn't yank the view around.
function setWsZoom(newZoom, clientX, clientY){
  newZoom = Math.max(WS_ZOOM_MIN, Math.min(WS_ZOOM_MAX, newZoom));
  if (Math.abs(newZoom - wsZoom) < 0.001) return;
  const vp = document.getElementById('ws-viewport');
  if (!vp) { wsZoom = newZoom; applyWsZoomToDOM(); return; }
  const vpRect = vp.getBoundingClientRect();
  const anchorX = clientX != null ? clientX : vpRect.left + vpRect.width / 2;
  const anchorY = clientY != null ? clientY : vpRect.top + vpRect.height / 2;
  const beforeX = vp.scrollLeft + (anchorX - vpRect.left);
  const beforeY = vp.scrollTop + (anchorY - vpRect.top);
  const logicalX = beforeX / wsZoom, logicalY = beforeY / wsZoom;
  wsZoom = newZoom;
  applyWsZoomToDOM();
  vp.scrollLeft = logicalX * wsZoom - (anchorX - vpRect.left);
  vp.scrollTop  = logicalY * wsZoom - (anchorY - vpRect.top);
}
// V2 item 6: the toolbar buttons used to step by a flat +/-0.1 (10 absolute
// percentage points) every click — a jarring ~40% relative jump at 25% zoom
// but a barely-there ~2% jump at 500%, and nothing like how the trackpad-
// pinch/ctrl+wheel handler (setWsZoom(wsZoom*factor,...), see the wheel
// listener above) actually feels. Switched to the same multiplicative model
// — a fixed ~15% relative step at any zoom level — so button-zoom and
// pinch-zoom feel consistent with each other, exactly as requested.
// direction: +1 to zoom in, -1 to zoom out.
const WS_ZOOM_STEP_FACTOR = 1.15;
// Zoom is a toolbar action too (section 8, Test J) — using it always leaves
// Point Edit mode, same as switching to any other primary tool.
function zoomWorkspace(direction){
  exitPointEditTool();
  setWsZoom(direction > 0 ? wsZoom * WS_ZOOM_STEP_FACTOR : wsZoom / WS_ZOOM_STEP_FACTOR);
}
function resetWorkspaceZoom(){ exitPointEditTool(); setWsZoom(1); }
...
// wsPanels is about to be replaced wholesale by a snapshot — any in-progress
// point-edit session is discarded (not committed: penEditPanelId's panel may
// not even exist, or may look different, in the snapshot being restored).
function _discardPenEditIfActive(){
  if (penEditMode) { penEditMode=false; penEditPanelId=null; clearPenState(); }
}
function performUndo(){
  if (!undoStack.length) return;
  _discardPenEditIfActive();
...
// ── Keyboard Movement (Workspace Enhancement, item 4) ──
const ARROW_KEYS = new Set(['ArrowUp','ArrowDown','ArrowLeft','ArrowRight']);
// Whether the current keyboard-nudge "gesture" (a key held down, which the
// browser auto-repeats as many keydown events) has already taken its one
// snapshotForUndo() — reset on keyup, so one hold = one undo step, exactly
// like one mouse-drag = one undo step (see onKeyUp).
let _kbMoveUndoSnapshotted = false;
// Moves every selected panel by (dx,dy) workspace-logical pixels — always
// in the workspace's fixed logical units, independent of the current zoom
// level, same as STEP sizes being fixed document units in real design
// tools. Updates the DOM immediately, same as a mouse-drag frame.
function moveSelectedPanelsBy(dx, dy){
  if (!selectedIds.size) return;
  if (!_kbMoveUndoSnapshotted) { snapshotForUndo(); _kbMoveUndoSnapshotted = true; }
...
// ── Point Edit tool (V4 redesign — Simplify Vector Point Editing) ──────────
// pointToolMode is now Point Edit's own primary-tool flag, on the same
// footing as selectMoveMode/penMode/freehandMode: exactly one of those four
// is ever the active primary tool (see exitPointEditTool(), called by every
// OTHER tool's activation so switching away always tears this down first —
// that centralised teardown is what section 8's bug report was missing).
// It's true in two situations: "armed" (participant clicked the Point Edit
// toolbar button but hasn't clicked a panel yet — see onWorkspaceMouseDown)
// and "editing" (penEditMode is also true — a specific panel's anchors are
// on screen). Both double-click and toolbar-then-click entry converge on
// enterPenEditMode(), which sets pointToolMode=true itself, so from the
// participant's perspective double-click always visibly activates the
// Point Edit toolbar button too (section 11) — there is no hidden state.
let pointToolMode   = false;
// Which click behaviour is armed while editing (pointEditMode===true).
// 'move' (default): click an anchor to select+drag it (a plain click
// without dragging toggles corner↔smooth — existing mechanism, reused
// unchanged in onPenMouseUp). 'add'/'delete': explicit, persistent
// sub-modes the participant switches into via their own toolbar buttons —
// they do NOT auto-revert after one use, so the sub-toolbar's highlighted
// button always matches what a click will actually do (section 7).
let pointEditAction = 'move'; // 'move' | 'add' | 'delete'
// The anchor most recently selected in Point Edit mode — purely a
// rendering concern (renderPenHandles() highlights it distinctly), not
// part of the path data itself. Persists after mouseup so "the anchor
// remains visible after selection" (section 3) rather than only being
// visible while the mouse button is held down. -1 = nothing selected.
let pointEditSelectedNode = -1;
const PEN_INSERT_HIT_R = 18; // px — how close a click needs to be to a segment to insert an anchor there
...
function _wireListeners(){
  ...
  // Hint Viewer — its own drag-to-pan/wheel-to-zoom, entirely separate from
  // the workspace's/garment-overlay's own mouse handling above.
  document.getElementById('hint-viewer-canvas').addEventListener('mousedown', onHintViewerMouseDown);
  document.addEventListener('mousemove', onHintViewerMouseMove);
  document.addEventListener('mouseup',   onHintViewerMouseUp);
  document.getElementById('hint-viewer-canvas').addEventListener('wheel', onHintViewerWheel, { passive:false });
  ...
  document.getElementById('ruler-h').addEventListener('mousedown', e => {
    e.preventDefault();
    const pos = Math.max(0, Math.min(WS_H, wsPointFromClient(e.clientX, e.clientY).y));
  ...
  document.getElementById('ruler-v').addEventListener('mousedown', e => {
    e.preventDefault();
    const pos = Math.max(0, Math.min(WS_W, wsPointFromClient(e.clientX, e.clientY).x));
  ...
  // Viewport scroll → sync all 4 rulers (also carries the current zoom scale — see syncRulerTransforms)
  document.getElementById('ws-viewport').addEventListener('scroll', syncRulerTransforms);

  // Workspace zoom — trackpad pinch and Ctrl/Cmd+wheel arrive as the exact
  // same event (browsers synthesize a wheel event with ctrlKey:true for a
  // pinch gesture), so one listener covers both. e.preventDefault() also
  // stops the browser's own page-zoom from firing at the same time.
  document.getElementById('ws-viewport').addEventListener('wheel', function(e){
    if (!(e.ctrlKey || e.metaKey)) return;
    e.preventDefault();
    const factor = Math.exp(-e.deltaY * 0.01);
    setWsZoom(wsZoom * factor, e.clientX, e.clientY);
  }, { passive:false });
...
function onKeyDown(e) {
  // Closing the Hint Viewer is inspection, not editing — stays available in
  // Review Submission (read-only) mode, so this check comes before the
  // psReadOnly early-return just below.
  if (e.key==='Escape' && document.getElementById('hint-viewer')?.classList.contains('visible')) {
    closeHintViewer();
    return;
  }
  ...
  if ((e.key==='Delete'||e.key==='Backspace') && selectedIds.size) {
    ...
    // Armed but not currently editing a panel (pointToolMode true, penEditMode
    // false) — Escape fully leaves the tool, same as clicking it again.
    if (pointToolMode) { togglePointTool(); return; }
    ...
    e.preventDefault();
    performUndo();
  } else if (ARROW_KEYS.has(e.key) && selectedIds.size && !penMode && !penEditMode) {
    // Keyboard Movement (Workspace Enhancement, item 4) — if nothing is
    // selected, or a drawing tool is active, arrow keys fall through to
    // whatever the browser would normally do with them (e.g. scrolling).
    if (document.activeElement.tagName==='INPUT'||document.activeElement.tagName==='TEXTAREA') return;
    e.preventDefault();
    const step = e.shiftKey ? 10 : 1;
    const dx = e.key==='ArrowLeft' ? -step : e.key==='ArrowRight' ? step : 0;
    const dy = e.key==='ArrowUp'   ? -step : e.key==='ArrowDown'  ? step : 0;
    moveSelectedPanelsBy(dx, dy);
  } else if (e.key===' ') {
    // V2 item 11 (Remove Spacebar Pan) — no dedicated pan tool is wired to
    // Space anywhere in this codebase; the "unreliable pan" participants
    // ran into was almost certainly the browser's own default Space
    // behavior (page scroll, or re-activating whatever toolbar button last
    // had focus) firing unpredictably while working in the canvas.
    // Neutralized outright, rather than left as a confusing, half-working
    // side effect.
    if (document.activeElement.tagName==='INPUT'||document.activeElement.tagName==='TEXTAREA') return;
    e.preventDefault();
  }
...
function onKeyUp(e) {
  ...
  // Ends the current keyboard-nudge "gesture" — see moveSelectedPanelsBy().
  // The browser auto-repeats keydown while a key is held, so releasing it
  // is the natural equivalent of a mouse-drag's mouseup.
  if (ARROW_KEYS.has(e.key)) _kbMoveUndoSnapshotted = false;
}
...
function renderPenHandles() {
  ...
  penNodes.forEach((n, i) => {
    ...
    // Anchor node — circle if smooth, square if corner. A selected anchor
    // (pointEditSelectedNode, see its declaration) gets a distinct blue
    // highlight — matching the handle-circle blue — so "select a point;
    // see that it is selected" (section 4) holds even before any drag, and
    // stays true after mouseup rather than only while dragging.
    const closeHover = isFirst && penNodes.length > 2 && penHoverClose;
    const isSelected = penEditMode && i === pointEditSelectedNode;
    const isSmooth = !!(n.handleIn || n.handleOut);
    if (isSmooth) {
      const circ = document.createElementNS(ns, 'circle');
      circ.setAttribute('cx', pf(n.x)); circ.setAttribute('cy', pf(n.y)); circ.setAttribute('r', isSelected ? '7' : '6');
      circ.setAttribute('fill',   closeHover ? 'rgba(45,190,108,.35)' : isSelected ? '#cfe4ff' : penEditMode ? '#fff0cc' : isLast ? '#fff7dd' : '#fff');
      circ.setAttribute('stroke', closeHover ? '#2dbe6c' : isSelected ? '#2288ff' : '#a06820');
      circ.setAttribute('stroke-width', isSelected ? '2.5' : '1.5');
      circ.style.cursor = 'pointer';
      circ.dataset.nodeIdx = i;
      nG.appendChild(circ);
    } else {
      const rSz = isSelected ? 6 : 5;
      const rect = document.createElementNS(ns, 'rect');
      rect.setAttribute('x', pf(n.x - rSz)); rect.setAttribute('y', pf(n.y - rSz));
      rect.setAttribute('width', rSz*2); rect.setAttribute('height', rSz*2);
      rect.setAttribute('fill',   closeHover ? 'rgba(45,190,108,.35)' : isSelected ? '#cfe4ff' : penEditMode ? '#fff0cc' : isLast ? '#fff7dd' : '#fff');
      rect.setAttribute('stroke', closeHover ? '#2dbe6c' : isSelected ? '#2288ff' : '#a06820');
...
// Shared by double-click-to-edit and the dedicated Point Editing Tool
// (V2 items 3/4) — converts the panel's SVG source into editable pen nodes
// on first use if it isn't pen data yet, then enters edit mode. Pulled out
// of the dblclick handler so both entry points behave identically.
function startEditingPanel(panelId){
  const q = wsPanels.find(q=>q.id===panelId);
  // Sketch objects (Freehand Sketch tool, V3 item 5) are exploratory
  // drawing, never garment-panel geometry — refuse to enter point-edit
  // mode on one even if this got called directly rather than through one
  // of the two isSketch-guarded entry points above.
  if (!q || q.isSketch) return;
  if (q.penData) { enterPenEditMode(panelId); return; }
  svgPanelToPenNodes(q).then(result => {
    if (!result) {
      alert("This panel doesn't have extractable outline data to edit (it's likely a photo/scan rather than a vector shape).");
      return;
    }
    snapshotForUndo();
    q.penData = { nodes: result.nodes, closed: result.closed };
    enterPenEditMode(panelId);
  });
}
function enterPenEditMode(panelId) {
  ...
  // Activate Point Edit — NOT Pen (V4 redesign: this used to set penMode=true
  // and highlight the Pen button, which is exactly the "secretly activates
  // Pen mode" confusion the interaction-model redesign fixes). Both entry
  // paths (double-click and toolbar Point Edit → click a panel) converge
  // here, so pointToolMode/btn-point become the visible, single source of
  // truth regardless of how editing was entered (section 11).
  pointToolMode = true;
  pointEditAction = 'move';
  pointEditSelectedNode = -1;
  selectMoveMode = false;
  document.getElementById('btn-point')?.classList.add('on');
  document.getElementById('btn-select')?.classList.remove('on');
  const _sub = document.getElementById('point-edit-subtools');
  if (_sub) _sub.style.display = 'flex';
  _updatePointEditSubtoolButtons();
  ...
  document.getElementById('workspace')?.classList.add('path-edit-active');
  renderPenPath();
  renderPenHandles();
  updateToolbarHint();
}
...
// Ends editing of THIS panel only — the Point Edit tool itself stays armed
// (pointToolMode untouched) so the participant can click straight into
// another panel, matching the tool's existing "stays active across panels"
// design (section 2 Option B / section 9). Switching to a different
// PRIMARY tool is a separate, explicit action — see exitPointEditTool(),
// which additionally clears pointToolMode and is what every other tool's
// activation calls first.
function exitPenEditMode() {
  ...
  document.getElementById('workspace')?.classList.remove('path-edit-active');
  updateToolbarHint();
}
...
function buildPanelTray(panels) {
  ...
  panels.forEach(p => {
    ...
    el.title = 'Click to insert into the workspace';
    el.innerHTML = `<img src="${p.src}" alt="" draggable="false">${p.label?`<div class="tray-panel-label">${_escLabel(p.label)}</div>`:''}`;
    // V2 item 2: a single click now inserts immediately (used to only show
    // a trace-guide overlay, requiring a separate double-click to actually
    // place it — that two-step overlay flow is no longer part of the
    // participant workflow). No dblclick listener anymore: with a single
    // click already inserting, a double-click would otherwise fire the
    // click handler twice AND then this handler, placing the panel 2-3
    // times over. showTraceOverlay()/closeTraceOverlay()/placeTracePanel()
    // are left intact but are now unreachable from the tray, same as
    // Merge/Trace's removal last round.
    el.addEventListener('click', () => addPanelToWorkspace(p.src));
    tray.appendChild(el);
  });
}
...
// ════════════════════════════════════════
//  HINT VIEWER — pannable/zoomable/dismissible overlay for opening a hint.
//  Independent of #garment-overlay/toggleGarmentOverlay() on purpose: that
//  element is shared by Map Mode's click-to-place-pin interaction and the
//  Primary-reference trace overlay, neither of which should ever have to
//  account for a pan/zoom transform. This is its own small, self-contained
//  viewer, opened/closed alongside the existing hint-reveal logic in
//  _onReferenceTabClick — never a replacement for it.
// ════════════════════════════════════════
let hintViewerZoom = 1, hintViewerPanX = 0, hintViewerPanY = 0;
let hintViewerDrag = null; // {sx, sy, startPanX, startPanY} while dragging
const HINT_VIEWER_ZOOM_MIN = 0.5, HINT_VIEWER_ZOOM_MAX = 4;
function openHintViewer(label, src){
  const el = document.getElementById('hint-viewer');
  if (!el || !src) return;
  hintViewerZoom = 1; hintViewerPanX = 0; hintViewerPanY = 0;
  document.getElementById('hint-viewer-img').src = src;
  document.getElementById('hint-viewer-title').textContent = label || '';
  _applyHintViewerTransform();
...
function _applyHintViewerTransform(){
  const img = document.getElementById('hint-viewer-img');
  if (img) img.style.transform = `translate(${hintViewerPanX}px, ${hintViewerPanY}px) scale(${hintViewerZoom})`;
  const pct = document.getElementById('hint-viewer-zoom-pct');
  if (pct) pct.textContent = Math.round(hintViewerZoom*100)+'%';
}
function hintViewerZoomBy(factor){
  hintViewerZoom = Math.max(HINT_VIEWER_ZOOM_MIN, Math.min(HINT_VIEWER_ZOOM_MAX, hintViewerZoom*factor));
  _applyHintViewerTransform();
}
function hintViewerResetZoom(){
  hintViewerZoom = 1; hintViewerPanX = 0; hintViewerPanY = 0;
  _applyHintViewerTransform();
}
function onHintViewerWheel(e){
  e.preventDefault();
  hintViewerZoomBy(e.deltaY < 0 ? 1.12 : (1/1.12));
}
function onHintViewerMouseDown(e){
  if (e.target.closest('.hint-viewer-topbar')) return;
  hintViewerDrag = { sx:e.clientX, sy:e.clientY, startPanX:hintViewerPanX, startPanY:hintViewerPanY };
  e.preventDefault();
}
function onHintViewerMouseMove(e){
  if (!hintViewerDrag) return;
  hintViewerPanX = hintViewerDrag.startPanX + (e.clientX - hintViewerDrag.sx);
  hintViewerPanY = hintViewerDrag.startPanY + (e.clientY - hintViewerDrag.sy);
  _applyHintViewerTransform();
}
function onHintViewerMouseUp(){ hintViewerDrag = null; }

// ════════════════════════════════════════
//  PEN MODE TOGGLE + EVENTS
// ════════════════════════════════════════
// ── Freehand Sketch tool (V3 item 5) — a pure pencil/annotation surface,
// deliberately NOT a shape-drawing tool. Strokes are never closed into a
// vector panel, never gain a fill, never get pen control points — they're
// open-ended polylines only. Multiple strokes drawn while the tool stays
// active accumulate into one "sketch session"; only when the participant
// switches to another tool does that whole session commit as a single
// object (see _exitFreehandMode/_commitFreehandSession). That object is
// still a normal wsPanels entry — reusing 100% of the existing move/select/
// delete/PNG-export/read-only machinery — just flagged isSketch:true so
// renderPanel() hides the panel-only Flip/Fill controls for it, and
// startEditingPanel()/the Point Editing Tool refuse to enter point-edit
// mode on it. Never converted into panel geometry (no penData, ever). ──
let freehandMode = false;
let freehandDrawing = false;
let freehandRawPoints = [];       // points of the stroke currently being drawn (mouse still down)
let freehandSession = [];         // strokes already finished this session, each an array of {x,y}
...
function toggleFreehandMode() {
  ...
  if (freehandMode) {
    _exitFreehandMode();
    // Same "return to the safe default" behaviour toggleSelectMoveMode()
    // documents — turning the drawing tool off always lands back on
    // Select & Move, there's no "neither" state once work has started.
    selectMoveMode = true;
  ...
  clearSelection();
  selectMoveMode = false;
  document.getElementById('btn-freehand')?.classList.add('on');
  document.getElementById('btn-select')?.classList.remove('on');
  const ws = document.getElementById('workspace');
  ws.classList.add('pen-mode', 'pen-mode-panels-off', 'freehand-mode');
  updateToolbarHint();
}
// Commits whatever was sketched this session (if anything) as one object,
// then fully resets freehand state. Shared by toggleFreehandMode() (normal
// tool switch) and setReadOnly() (Finish clicked mid-sketch) so there's one
// single place that decides "the sketch session is over."
function _exitFreehandMode(){
  _commitFreehandSession();
  freehandMode = false;
  freehandDrawing = false;
  freehandRawPoints = [];
  freehandSession = [];
  document.getElementById('btn-freehand')?.classList.remove('on');
  document.getElementById('workspace')?.classList.remove('pen-mode', 'pen-mode-panels-off', 'freehand-mode');
...
function freehandPointerDown(e) {
  ...
  freehandRawPoints = [];
  freehandRawPoints.push(wsPointFromClient(e.clientX, e.clientY));
  e.preventDefault(); e.stopPropagation();
}
function freehandPointerMove(e) {
  if (!freehandMode || !freehandDrawing) return;
  const { x, y } = wsPointFromClient(e.clientX, e.clientY);
...
function freehandPointerUp(e) {
  ...
  // A single click (no real drag) isn't a stroke — nothing to add.
  if (simplified.length >= 2) freehandSession.push(simplified);
  _renderFreehandPreview();
  e.stopPropagation();
}
// One <path> "d" can hold several disconnected subpaths (repeated M...L...
// groups) — used here to show every stroke finished so far in this session
// plus the one currently being drawn, all at once, without needing a
// separate SVG element per stroke while still in progress.
function _renderFreehandPreview() {
  const path = document.getElementById('pen-path');
  if (!path) return;
  const subpaths = freehandSession
    .concat(freehandRawPoints.length >= 2 ? [freehandRawPoints] : [])
    .filter(pts => pts.length >= 2)
    .map(pts => 'M' + pts.map(p => p.x.toFixed(1) + ',' + p.y.toFixed(1)).join(' L'));
  if (!subpaths.length) { path.style.display = 'none'; path.setAttribute('d', ''); return; }
...
// Builds the one committed sketch object from every stroke drawn this
// session — stroke-only <path> elements (fill:none, rounded caps/joins for
// a pencil feel), never a filled/closed shape, never pen nodes. Still just
// an ordinary image src on an ordinary wsPanels entry, so it's selectable/
// movable/deletable/exported exactly like a panel — isSketch:true is the
// only thing that tells renderPanel()/startEditingPanel() to treat it
// differently (see their own comments).
function _commitFreehandSession(){
  if (!freehandSession.length) return;
  const allPts = freehandSession.flat();
  const xs = allPts.map(p => p.x), ys = allPts.map(p => p.y);
  const pad = 10;
  const minX = Math.min(...xs) - pad, minY = Math.min(...ys) - pad;
  const w = Math.max(...xs) - Math.min(...xs) + pad * 2;
  const h = Math.max(...ys) - Math.min(...ys) + pad * 2;
  const paths = freehandSession.map(pts => {
    const d = 'M' + pts.map(p => (p.x - minX).toFixed(1) + ',' + (p.y - minY).toFixed(1)).join(' L');
    return `<path d="${d}" fill="none" stroke="#3a3a3a" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"/>`;
  }).join('');
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="${w.toFixed(1)}" height="${h.toFixed(1)}">${paths}</svg>`;
  snapshotForUndo();
  const np = {
    id: 'wp_sketch_' + Date.now() + '_' + Math.random().toString(36).slice(2, 5),
    src: 'data:image/svg+xml;base64,' + btoa(svg),
    x: minX, y: minY, rot: 0, flip: false, scale: w / 100,
    isSketch: true,
  };
  wsPanels.push(np); renderPanel(np); updateEmptyState(); bringToTop(np.id);
}

// ── Point Edit teardown (V4 redesign — section 8): every OTHER primary
// tool's activation calls this FIRST, so switching away from Point Edit is
// never something an individual tool-toggle function has to remember to
// handle piecemeal (that was the actual bug — most of them only remembered
// half of penEditMode/penMode/pointToolMode). Commits whatever's being
// edited (never discards silently), then fully deactivates the tool:
// un-highlights btn-point, hides the Move/Add/Delete sub-toolbar, and
// resets the sub-mode back to its default for next time. ──
function exitPointEditTool() {
  if (penEditMode) commitPenEdit();
  if (!pointToolMode) return;
  pointToolMode = false;
  pointEditAction = 'move';
  pointEditSelectedNode = -1;
  document.getElementById('btn-point')?.classList.remove('on');
  const sub = document.getElementById('point-edit-subtools');
  if (sub) sub.style.display = 'none';
}
function _updatePointEditSubtoolButtons() {
  document.getElementById('btn-point-move')?.classList.toggle('on', pointEditAction==='move');
  document.getElementById('btn-point-add')?.classList.toggle('on', pointEditAction==='add');
  document.getElementById('btn-point-delete')?.classList.toggle('on', pointEditAction==='delete');
}
// Move/Add/Delete sub-toolbar buttons, only reachable while Point Edit is
// the active primary tool — see the toolbar markup's #point-edit-subtools.
function setPointEditAction(action) {
  if (!pointToolMode) return;
  pointEditAction = action;
  pointEditSelectedNode = -1;
  renderPenHandles();
  _updatePointEditSubtoolButtons();
  updateToolbarHint();
...
// ── Point Edit tool (V4 redesign): click a panel (or double-click one
// directly — see enterPenEditMode()) to edit its anchors. Default click
// behaviour is Move (select + drag an anchor); Add Point/Delete Point are
// explicit sub-modes armed via their own toolbar buttons (setPointEditAction).
// Unlike Pen/Freehand, this tool stays active across panels: finishing one
// panel's edit (click outside it, or Esc) leaves the tool armed to click the
// next panel — exiting the tool entirely is only ever a deliberate toolbar
// click, matching the pre-existing V2 design. ──
...
// ════════════════════════════════════════
//  POINT EDITING TOOL — insert/delete anchors (V2 item 4)
// ════════════════════════════════════════

// De Casteljau subdivision of a cubic bezier (p0,c1,c2,p3) at parameter t —
// splits the curve into two cubics that together retrace the original
// exactly, so inserting an anchor never visibly reshapes the path.
function _splitCubicBezier(p0, c1, c2, p3, t) {
  const lerp = (a,b,t) => ({ x: a.x+(b.x-a.x)*t, y: a.y+(b.y-a.y)*t });
  const ab = lerp(p0,c1,t), bc = lerp(c1,c2,t), cd = lerp(c2,p3,t);
  const abc = lerp(ab,bc,t), bcd = lerp(bc,cd,t);
  const abcd = lerp(abc,bcd,t);
  return { left: [p0, ab, abc, abcd], right: [abcd, bcd, cd, p3] };
}

// Finds the path segment nearest (x,y) by sampling each segment (straight or
// curved) and projecting onto the resulting chords. Returns
// {segIndex, t, dist} for the closest sample, or null if nothing is within
// PEN_INSERT_HIT_R of the click.
function _nearestPenSegment(x, y) {
  const n = penNodes.length;
  const segCount = penEditClosed ? n : n - 1;
  let best = null;
  const SAMPLES = 24;
  for (let i = 0; i < segCount; i++) {
    const a = penNodes[i], b = penNodes[(i+1) % n];
    const cp1 = a.handleOut, cp2 = b.handleIn;
    let prevPt = a, prevT = 0;
    for (let s = 1; s <= SAMPLES; s++) {
      ...
      if (!cp1 && !cp2) {
        ...
        const c1x = cp1?cp1.x:a.x, c1y = cp1?cp1.y:a.y, c2x = cp2?cp2.x:b.x, c2y = cp2?cp2.y:b.y;
        const mt = 1-t;
        pt = {
      ...
      let proj = len2>0 ? ((x-prevPt.x)*dx+(y-prevPt.y)*dy)/len2 : 0;
      proj = Math.max(0, Math.min(1, proj));
      const cx = prevPt.x+dx*proj, cy = prevPt.y+dy*proj;
      const dist = Math.hypot(x-cx, y-cy);
      if (!best || dist < best.dist) best = { segIndex: i, t: prevT+(t-prevT)*proj, dist };
      prevPt = pt; prevT = t;
    }
  }
  return (best && best.dist <= PEN_INSERT_HIT_R) ? best : null;
}

// Insert a new anchor on segment segIndex at parameter t, splitting any
// bezier handles via _splitCubicBezier so the visible curve is unchanged.
function insertPenAnchorAtSegment(segIndex, t) {
  const n = penNodes.length;
  const a = penNodes[segIndex], b = penNodes[(segIndex+1) % n];
  const cp1 = a.handleOut, cp2 = b.handleIn;
  let newNode;
  if (!cp1 && !cp2) {
    newNode = { x: a.x+(b.x-a.x)*t, y: a.y+(b.y-a.y)*t, smooth:false, handleIn:null, handleOut:null };
  } else {
    const c1 = cp1 || {x:a.x,y:a.y}, c2 = cp2 || {x:b.x,y:b.y};
    const { left, right } = _splitCubicBezier(a, c1, c2, b, t);
    a.handleOut = (left[1].x===a.x && left[1].y===a.y) ? null : { x:left[1].x, y:left[1].y };
    b.handleIn  = (right[2].x===b.x && right[2].y===b.y) ? null : { x:right[2].x, y:right[2].y };
    newNode = {
      x: left[3].x, y: left[3].y, smooth: true,
      handleIn:  { x:left[2].x,  y:left[2].y },
      handleOut: { x:right[1].x, y:right[1].y },
    };
  }
  penNodes.splice(segIndex+1, 0, newNode);
}

// Delete the anchor at nodeIndex — refuses to collapse the shape below 3
// nodes (closed path) or 2 nodes (open path).
function deletePenAnchorAt(nodeIndex) {
  const minNodes = penEditClosed ? 3 : 2;
  if (penNodes.length <= minNodes) return false;
  penNodes.splice(nodeIndex, 1);
  return true;
}

// "Click outside the panel" exit check for edit mode (V2 item 3) — outside
// the pen nodes' bounding box (with a small margin so near-edge clicks still
// count as "on" the shape) counts as outside.
function _isOutsidePenBounds(x, y, pad=20) {
  if (!penNodes.length) return true;
...
function onPenMouseDown(e) {
  ...
  // ── EDIT MODE (V4 redesign: routes on pointEditAction, not a separate
  // "toolbar tool vs double-click" split — both entry paths share this one
  // set of behaviours now) ──
  if (penEditMode) {
    // Add Point — the next click on the path inserts a new anchor there.
    // Stays armed after use (section 7: no silent mode changes) — the
    // participant switches back to Move explicitly when done adding.
    if (pointEditAction === 'add') {
      const seg = _nearestPenSegment(x, y);
      if (seg) {
        snapshotForUndo();
        insertPenAnchorAtSegment(seg.segIndex, seg.t);
        pointEditSelectedNode = seg.segIndex + 1; // the just-inserted anchor
        renderPenPath(); renderPenHandles(); liveUpdatePenPanel();
      }
      // A click that doesn't land near the path is simply a no-op here —
      // never falls through to closing/inserting elsewhere or exiting.
      return;
    }
    // Delete Point — click an existing anchor to remove it. Also stays
    // armed after use, same reasoning as Add Point above.
    if (pointEditAction === 'delete') {
      const nHit = hitTestNode(x, y, PEN_NODE_HIT_R);
      if (nHit >= 0) {
        snapshotForUndo();
        if (deletePenAnchorAt(nHit)) {
          pointEditSelectedNode = -1;
          renderPenPath(); renderPenHandles(); liveUpdatePenPanel();
        }
      }
    ...
    // Move (default) — click an anchor to select+drag it (a plain click
    // without dragging toggles corner↔smooth, handled in onPenMouseUp).
    // Handle hit?
    const hHit = hitTestHandle(x, y, PEN_HANDLE_HIT_R);
    if (hHit) { penEditDragHandle = hHit; return; }
    // Node hit?
    const nHit = hitTestNode(x, y, PEN_NODE_HIT_R);
    if (nHit >= 0) { penEditDragNode = nHit; penEditNodeStart = { x, y }; pointEditSelectedNode = nHit; renderPenHandles(); return; }
    // Click outside the shape (V2 item 3) — exit edit mode
    if (_isOutsidePenBounds(x, y)) { commitPenEdit(); return; }
...
function onPenMouseUp(e) {
  ...
  if (penEditMode) {
    if (penEditDragNode >= 0 && penEditNodeStart) {
      const { x: ux, y: uy } = wsPointFromClient(e.clientX, e.clientY);
  ...
  if (penDragging) {
    ...
    const { x: px, y: py } = wsPointFromClient(e.clientX, e.clientY);
    document.getElementById('pen-preview').setAttribute('d', buildPenPreviewD(px, py));
  }
}
...
function applyTaskFile(t){
  ...
  const g = t.garment || {};
  currentTaskMeta = t;
  // Stage 2 ("Construct the garment") is a physical/Clo3D construction
  // activity — the drawing/editing toolbars and Full Screen don't apply, but
  // the task/hint panel and the workspace itself (drag panels around for
  // reference) stay exactly as they are for every other stage. See
  // showCompleteModal() for the matching Finish-button change.
  const isConstructionStage = t.stage === 's2';
  document.getElementById('screen-workshop')?.classList.toggle('ps-construction-mode', isConstructionStage);
  if (isConstructionStage && psFullscreen) toggleWorkspaceFullscreen();
...
function _onReferenceTabClick(idx){
  ...
  if (tab.kind==='primary'){ _setActiveReferenceView(null, tab.src); closeHintViewer(); return; }
  if (tab.kind==='panels'){
  ...
  if (referenceActiveKey === tab.key){
    // Hide → revert to Primary and close the Hint Viewer. Not logged while
    // read-only — switching during review is viewing, not a new
    // behavioural data point.
    ...
    // "Opening a hint should overlay it on the workspace" — clicking a hint
    // tab both reveals/logs it (above) and immediately opens the pannable/
    // zoomable viewer; no separate click needed.
    openHintViewer(tab.label, tab.src);
  }
}
function _escLabel(s){ return String(s==null?'':s).replace(/[&<>"']/g, c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c])); }
function deleteSelectedPanels(){
  if (!selectedIds.size) { _healDesyncedInteractionClasses(); alert('Select one or more panels first, then click Delete.'); return; }
  snapshotForUndo();
  [...selectedIds].forEach(id=>{
...
function renderPlanPanel(t){
  ...
  const refs = t.references;
  const cardImg = refs ? _referenceImageSrc(refs.primary) : (g.image || '');
  // Every real research stage always has non-empty t.references (see
  // STAGE_REFERENCES in participant-page_3.html) — this branch is only ever
  // taken by the Training Task, which deliberately has none, so hiding the
  // Hints section here can never affect a real stage.
  el.innerHTML = `
    ...
    ${(t.stage==='s2' && t.fullArrangementPdf) ? `<a class="ps-pdf-download" href="${t.fullArrangementPdf}" download="${_escLabel(t.fullArrangementPdfName||'full-arrangement-reference.pdf')}">⤓ Download Full Panel Arrangement PDF</a>` : ''}
    ${refs ? `<div class="ps-hints-eyebrow">Hints</div>
    <div class="ps-image-tabs" id="ps-image-tabs"></div>
    <p class="ps-brief-note">Hints are optional — open one only if you want more help. Once opened, it stays available.</p>` : ''}
  `;
}
...
function tickTimers() {
  ...
  if (!psReadOnly && workshopStarted && currentTaskMeta && currentTaskMeta.budgetMin) {
    ...
    const timeUp = elapsed >= budgetMs;
    const tEl = document.getElementById('ps-timer');
    // Purely presentational (no auto-lock, no auto-Finish, no auto-download —
    // Finish stays a participant action regardless of the countdown). Applies
    // to every stage with a budget, not just Stage 2 — one shared countdown
    // widget, so this reads clearly wherever it's used.
    if (tEl) { tEl.textContent = timeUp ? 'Time’s up' : formatTime(remain); tEl.classList.toggle('time-up', timeUp); }
...
function duplicateSelectedPanels(){
  if (!selectedIds.size) { _healDesyncedInteractionClasses(); alert('Select one or more panels first, then click Duplicate.'); return; }
  snapshotForUndo();
...
function rotateSelectedPanels(deg){
  if (!selectedIds.size) { _healDesyncedInteractionClasses(); alert('Select one or more panels first, then click Rotate.'); return; }
  snapshotForUndo();
  let skippedMerged = 0;
  selectedIds.forEach(id=>{
...
function flipSelectedPanels(){
  if (!selectedIds.size) { _healDesyncedInteractionClasses(); alert('Select one or more panels first, then click Flip.'); return; }
  snapshotForUndo();
  selectedIds.forEach(id=>{
...
// ── Full-screen workspace (Workspace Enhancement, item 1) — an in-page
// layout expansion, not the browser's native Fullscreen API: hides the
// task-brief panel, panel tray and canvas hint so the workspace fills
// nearly the whole window, while the participant stays on the exact same
// activity/screen the whole time. Purely a CSS-class toggle — no state is
// persisted, so it always resets to normal on leaving/reloading the
// activity, and it doesn't touch save/export/workflow logic at all. ──
let psFullscreen = false;
function toggleWorkspaceFullscreen(){
  psFullscreen = !psFullscreen;
  document.getElementById('screen-workshop')?.classList.toggle('ps-fullscreen', psFullscreen);
  const btn = document.getElementById('btn-fullscreen');
  if (btn) btn.innerHTML = psFullscreen
    ? '<i data-lucide="minimize-2"></i> Restore'
...
function updateToolbarHint() {
  ...
  if (!el) return;
  if (pointToolMode && penEditMode) {
    if (pointEditAction==='add') el.innerHTML='◈ <strong>Add Point:</strong> click anywhere on the path to insert a new anchor there';
    else if (pointEditAction==='delete') el.innerHTML='◈ <strong>Delete Point:</strong> click an anchor to remove it';
    else el.innerHTML='◈ <strong>Point Edit:</strong> click an anchor to select it, drag to move &nbsp;·&nbsp; click without dragging = toggle straight ↔ curved &nbsp;·&nbsp; click outside the shape or <kbd>Esc</kbd> = done with this panel';
  }
  else if (pointToolMode) el.innerHTML='◈ <strong>Point Edit:</strong> click a panel to start editing its points';
  else if (penMode) el.innerHTML='✒ <strong>Pen:</strong> <strong>click</strong> = corner node &nbsp;·&nbsp; <strong>click+drag</strong> = curved node with handles &nbsp;·&nbsp; hover first node (turns green) + click = close path &nbsp;·&nbsp; drag on close = curve the closing segment &nbsp;·&nbsp; <kbd>↵ Enter</kbd> or double-click = finish open &nbsp;·&nbsp; <kbd>Esc</kbd> = cancel &nbsp;·&nbsp; <strong>double-click finished shape</strong> = edit its points';
...
function hasOverlapAt(panelId, clientX, clientY) {
  const { x: wx, y: wy } = wsPointFromClient(clientX, clientY);
...
function renderPanel(p) {
  ...
  const w=Math.round(100*p.scale);
  // A sketch object (Freehand Sketch tool, V3 item 5) is exploratory
  // drawing/annotation, never garment-panel geometry — no Flip/Fill
  // controls, matching the explicit "no panel-specific Flip/Fill controls"
  // requirement. Select/Move/resize/delete stay identical to a real panel.
  const pctrlButtons = p.isSketch
  ...
  div.addEventListener('mousedown', e => {
    ...
    // Point Edit tool — clicking any panel while this tool is active jumps
    // straight into editing its points via enterPenEditMode() (same as a
    // double-click would), so the tool is immediately usable without an
    // extra step. Once editing, onPenMouseDown's penEditMode branch takes
    // over, routed by pointEditAction (move/add/delete). Sketch objects have
    // no control points to edit (V3 item 5) — a click just selects/drags
    // them like Select & Move would, same as every branch below this one.
    if (pointToolMode && !p.isSketch && p.id !== penEditPanelId) { startEditingPanel(p.id); return; }
    // A non-sketch panel already being edited — let onPenMouseDown/
    // onWorkspaceMouseDown handle the click instead of falling through to
    // drag/select below. A sketch under Point Tool has nothing to enter
    // edit mode on, so it deliberately skips this and falls through.
    if (pointToolMode && !p.isSketch) return;
  ...
  // Sketch objects have no such geometry to edit (V3 item 5) — a double-
  // click on one is just two selects in a row, nothing more. ──
  div.addEventListener('dblclick', e => {
    if (e.target.tagName==='BUTTON'||mapMode||p.isSketch) return;
    if (hasOverlapAt(p.id, e.clientX, e.clientY) && !isPointOpaqueOnPanel(p, e.clientX, e.clientY)) return;
    e.stopPropagation();
    startEditingPanel(p.id);
  });
  ...
  div.querySelector('.resize-handle').addEventListener('mousedown', e => {
    ...
    if (e.metaKey||e.ctrlKey) {
      ...
      // elRect/wsRect are both post-zoom screen rects — their difference is
      // a screen-pixel delta, so it needs the same /wsZoom as any other
      // client delta to land back in workspace-logical space (matching mwx/mwy below).
      const ws_cx=((elRect.left-wsRect.left)+elRect.width/2)/wsZoom;
      const ws_cy=((elRect.top-wsRect.top)+elRect.height/2)/wsZoom;
      const { x: mwx, y: mwy } = wsPointFromClient(e.clientX, e.clientY);
...
//  WORKSPACE MOUSEDOWN
// ════════════════════════════════════════
// Participant-safety self-heal: if every tool flag agrees we should be in
// plain Select mode, but the workspace still carries a drawing/edit-tool CSS
// class (pen-mode-panels-off disables pointer-events on every panel — the
// exact "clicking a visible object does nothing" symptom), that's a stray
// leftover from an interrupted tool switch. Strip just those classes — never
// touches wsPanels/selectedIds/undo history — so the very next click on a
// panel works normally again. See resetInteractionState() for the full
// manual recovery this mirrors a slice of.
function _healDesyncedInteractionClasses() {
  if (penMode || penEditMode || pointToolMode || freehandMode || mapMode) return;
  const ws = document.getElementById('workspace');
  if (ws && (ws.classList.contains('pen-mode') || ws.classList.contains('pen-mode-panels-off') || ws.classList.contains('freehand-mode') || ws.classList.contains('path-edit-active'))) {
    ws.classList.remove('pen-mode', 'pen-mode-panels-off', 'freehand-mode', 'path-edit-active');
    document.getElementById('pen-svg')?.classList.remove('edit-mode');
  }
}
function onWorkspaceMouseDown(e) {
  if (psReadOnly) return;
  _healDesyncedInteractionClasses();
  if (e.target.closest('#mannequin-win')) return;
  if (penMode || penEditMode) { onPenMouseDown(e); return; }
  if (pointToolMode) return; // armed but not editing a panel yet — click a panel to start
  ...
  const { x: sx, y: sy } = wsPointFromClient(e.clientX, e.clientY);
  rubberBand={ active:true, sx, sy };
...
function onMouseMove(e) {
  ...
  if (resizeDrag.active) {
    ...
    if (resizeDrag.mode==='rotate') {
      ...
      const { dx, dy } = wsDeltaFromClient(e.clientX-resizeDrag.sx, e.clientY-resizeDrag.sy);
  ...
  // Pen
  if (penMode || penEditMode) { onPenMouseMove(e); return; }
  // Rubber-band
  if (rubberBand.active) {
    const {sx,sy}=rubberBand;
    const {x:cx,y:cy}=wsPointFromClient(e.clientX, e.clientY);
  ...
  if (drag.active) {
    if (drag.group) {
      const { dx, dy } = wsDeltaFromClient(e.clientX-drag.sx, e.clientY-drag.sy);
...
function onMouseUp(e) {
  ...
  if (penMode || penEditMode) { onPenMouseUp(e); }
  if (rubberBand.active) {
...
document.addEventListener('touchmove',e=>{
  ...
  const { dx, dy } = wsDeltaFromClient(t.clientX-drag.sx, t.clientY-drag.sy);
  q.x=drag.px+dx; q.y=drag.py+dy;
  const el=document.getElementById(drag.id);
  if(el){el.style.left=q.x+'px';el.style.top=q.y+'px';}
},{passive:false});
...
// ── EMERGENCY STABILIZATION (participant-safety fix) ────────────────────
// Recovers from a stuck interaction state — panels visible but unclickable,
// selection/Delete/Rotate reporting "no object selected" — without ever
// touching wsPanels. Root cause: after several Select/Pen/Point-Edit/
// Freehand switches, a leftover tool flag or the `pen-mode-panels-off` CSS
// class (which disables pointer-events on every panel while a drawing/edit
// tool owns the workspace) can end up not cleared, silently blocking clicks
// even though nothing is visibly "active". This is the opposite of
// clearWorkspace(): it resets ONLY transient tool/pointer state — never
// wsPanels, never selection targets' geometry, never undo/redo history.
// Any in-progress edit is COMMITTED (not discarded) first, so a participant
// never loses a point-edit or freehand stroke they were mid-way through.
function resetInteractionState() {
  try { if (penEditMode) commitPenEdit(); } catch(e) { console.error('resetInteractionState: commitPenEdit failed', e); }
  try { if (freehandMode && freehandSession && freehandSession.length) _commitFreehandSession(); } catch(e) { console.error('resetInteractionState: freehand commit failed', e); }
  // An in-progress (uncommitted) Pen trace with real content is preserved as
  // an open shape rather than silently vanishing; a barely-started trace
  // (0-1 nodes) has nothing worth keeping.
  try { if (penMode && penNodes && penNodes.length >= 2) finishPen(false); } catch(e) { console.error('resetInteractionState: finishPen failed', e); }

  penMode = false; penEditMode = false; penEditPanelId = null;
  penEditDragNode = -1; penEditDragHandle = null; penEditNodeStart = null;
  penDragging = false; penDragOrigin = null; penPendingClose = false; penFinishing = false;
  pointToolMode = false; pointEditAction = 'move'; pointEditSelectedNode = -1;
  freehandMode = false; freehandDrawing = false; freehandRawPoints = []; freehandSession = [];
  if (mapMode) { try { exitMapMode(); } catch(e) {} }

  // Stale pointer-interaction refs (drag/resize/rubber-band/guide) — always
  // safe to drop; they only ever describe an in-flight mouse gesture, never
  // committed geometry.
  drag = { active:false };
  if (typeof resizeDrag !== 'undefined') resizeDrag = { active:false };
  if (typeof rubberBand !== 'undefined') rubberBand = { active:false };
  if (typeof guideDrag !== 'undefined') guideDrag = { active:false, id:null, type:null };

  penNodes = [];
  clearPenState();

  const ws = document.getElementById('workspace');
  if (ws) ws.classList.remove('pen-mode', 'pen-mode-panels-off', 'freehand-mode', 'path-edit-active', 'map-mode');
  document.getElementById('pen-svg')?.classList.remove('edit-mode');
  document.getElementById('sel-rect')?.style.setProperty('display', 'none');

  ['btn-draw','btn-point','btn-freehand','btn-map'].forEach(id => document.getElementById(id)?.classList.remove('on','mode-on'));
  const sub = document.getElementById('point-edit-subtools');
  if (sub) sub.style.display = 'none';
...
function clearWorkspace() {
  ...
  // Everything (including whatever's mid-edit) is about to be wiped — discard
  // any in-progress point-edit session directly rather than committing it
  // (there's nothing left to commit it onto a moment later).
  if (penEditMode) { penEditMode=false; penEditPanelId=null; }
  ...
  // A cleared workspace has nothing left to draw or edit — return to the
  // baseline Select & Move tool rather than leaving a stale Pen/Freehand/
  // Point Edit button highlighted after everything's been wiped.
  if (freehandMode) { freehandMode=false; document.getElementById('btn-freehand')?.classList.remove('on'); }
  if (penMode) { penMode=false; document.getElementById('btn-draw')?.classList.remove('on'); }
  if (pointToolMode) {
    pointToolMode = false; pointEditAction = 'move'; pointEditSelectedNode = -1;
    document.getElementById('btn-point')?.classList.remove('on');
    const sub = document.getElementById('point-edit-subtools');
  ...
  document.getElementById('workspace')?.classList.remove('pen-mode','pen-mode-panels-off','freehand-mode','path-edit-active');
  document.getElementById('pen-svg')?.classList.remove('edit-mode');
  selectMoveMode = true;
  document.getElementById('btn-select')?.classList.add('on');
  // Same "fresh start" treatment as the tool-mode reset above — also runs
  // on every new task load (applyTaskFile calls clearWorkspace first), so a
  // zoom level left over from a previous stage never carries over.
  resetWorkspaceZoom();
}
...
async function mergeSelectedPanels() {
  ...
  // Toolbar Simplification: no longer has a toolbar button (participant
  // workflow doesn't use Merge), but the implementation stays — see
  // window.mergeSelectedPanels export. Falls back to a throwaway object so
  // the textContent/disabled bookkeeping below stays harmless if this is
  // ever invoked without that button in the DOM.
  const btn = document.getElementById('btn-merge') || { textContent:'', disabled:false };
...
async function traceSelectedPanels() {
  ...
  // Toolbar Simplification: no longer has a toolbar button — see the same
  // note on mergeSelectedPanels() above.
  const tBtn = document.getElementById('btn-trace') || { textContent:'', disabled:false };
...
function svgWithFill(src, colour) {
  // Decode the SVG payload — Fill Tool investigation (V2 item 10) found this
  // only ever recognized two exact prefixes ("...svg+xml;base64," and bare
  // "...svg+xml,"), silently returning the source UNCHANGED for any other
  // parameter in between (most importantly "...svg+xml;charset=utf-8,",
  // which is exactly what shapeToDataURL() produces for every basic panel
  // used in Pre-test/Post-test/Stage 1-3's starting tray) — Fill looked like
  // it worked (a new src did get written, fill-color state did get stored)
  // but the actual image never changed. Now handles any
  // "data:image/svg+xml[;whatever]," variant: base64 iff the part before
  // the comma ends in ";base64", percent-encoded otherwise.
  let svgText = '';
  if (src.startsWith('data:image/svg+xml')) {
    ...
    try {
      svgText = /;base64$/.test(meta) ? atob(payload) : decodeURIComponent(payload);
    } catch(e) { return src; }
  } else {
...
function setReadOnly(ro, opts){
  psReadOnly = !!ro;
  if (psReadOnly) {
    // Defence-in-depth: force out of any in-progress edit/draw session so no
    // stray pointer-events:auto overlay (e.g. #pen-svg.edit-mode) is left
    // interactive once the workspace locks for review.
    if (penEditMode) commitPenEdit();
    if (penMode) { penMode=false; clearPenState(); document.getElementById('btn-draw')?.classList.remove('on'); document.getElementById('workspace')?.classList.remove('pen-mode','pen-mode-panels-off'); }
    // _exitFreehandMode() commits whatever was sketched this session as one
    // object before clearing state — Finish being clicked mid-sketch must
    // not silently lose it (a plain freehandMode=false here would).
    if (freehandMode) _exitFreehandMode();
    pointToolMode = false;
    pointEditAction = 'move';
    pointEditSelectedNode = -1;
    document.getElementById('btn-point')?.classList.remove('on');
...
window.openHintViewer = openHintViewer;
window.closeHintViewer = closeHintViewer;
window.hintViewerZoomBy = hintViewerZoomBy;
window.hintViewerResetZoom = hintViewerResetZoom;
window.toggleLeftPanel = toggleLeftPanel;
window.togglePenMode = togglePenMode;
window.togglePointTool = togglePointTool;
window.setPointEditAction = setPointEditAction;
window.resetInteractionState = resetInteractionState;
window.toggleSelectMoveMode = toggleSelectMoveMode;
window.toggleWorkspaceFullscreen = toggleWorkspaceFullscreen;
...
return { mount, applyTaskFile, buildStateSnapshot, restoreState, onComplete, isMounted, setReadOnly, onBack, setReferenceUnlockState, onReferenceEvent, shapeToDataURL };
})();
